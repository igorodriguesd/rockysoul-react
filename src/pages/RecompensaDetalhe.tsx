      ...
      <GlassCard className="p-6 sm:p-8">
        <div className="flex flex-col sm:flex-row items-start gap-4 sm:items-center mb-5">
          ...
          <div className="min-w-0">
            {recompensa.badge && (
              <span className="inline-block px-2 py-0.5 bg-[#22c55e]/15 text-[#4ade80] text-[10px] font-semibold rounded-full mb-1.5">
                {recompensa.badge}
              </span>
            )}
            <h1 className="text-2xl font-bold text-white leading-tight">{recompensa.nome}</h1>
            <span className="text-xs text-white/45 bg-white/10 px-2 py-0.5 rounded-full mt-1.5 inline-block">
              {recompensa.categoria}
            </span>
          </div>
        </div>

        <p className="text-sm text-white/60 leading-relaxed mb-6">{recompensa.descricao}</p>

        <div className="flex items-center justify-between rounded-xl bg-[#22c55e]/10 border border-[#22c55e]/20 px-4 py-3 mb-6">
          <div>
            <p className="text-[10px] text-white/45">Seu saldo</p>
            <p className="text-base font-bold text-white">{data.pontos} pts</p>
          </div>
          <div className="text-right">
            <p className="text-[10px] text-white/45">Custo</p>
            <p className="text-base font-bold text-[#4ade80]">{recompensa.pontos} pts</p>
          </div>
        </div>
